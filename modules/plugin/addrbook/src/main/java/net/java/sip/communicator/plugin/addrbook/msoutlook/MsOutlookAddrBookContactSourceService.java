    ...
    implements EditableContactSourceService, PrefixedContactSourceService
{
    ...
    static
    {
        ...
        {
            logger.error(
                    "Failed to load Outlook native library: "
                        + t.getMessage());
        }
        ...
        if (loaded)
        {
            ...
            catch (ClassNotFoundException | UnsatisfiedLinkError cnfe)
            {
                loaded = false;
                logger.error("Could not load query", cnfe);
            }
