    ...
    implements CustomResolver, PropertyChangeListener
{
     ...
     */
    ParallelResolverImpl(ConfigurationService configService)
    {
        this.configService = configService;
        backupQueriesPool = Executors.newCachedThreadPool();
        configService.addPropertyChangeListener(this);
        initProperties();
        reset();
    }
    ...
    private void initProperties()
    {
        ...
        int resolverPort = configService.getInt(
        ...
        currentDnsPatience = configService
            .getLong(PNAME_DNS_PATIENCE, DNS_PATIENCE);

        currentDnsRedemption
            = configService.getInt(PNAME_DNS_REDEMPTION, DNS_REDEMPTION);
    }
